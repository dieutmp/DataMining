...
public class DefaultOrNotClassifier{
	Instances train;
	Instances test;
	Evaluation eval;
	FilteredClassifier fc;
	
	public DefaultOrNotClassifier(String trainFile, String testFile, Classifier cls) throws Exception{
		...
		for (int i = 0; i < train.numAttributes(); i++){
			...
			if (name.startsWith("default.or.not")){
				classIndex = i;
			}else if (name.startsWith("collet") || name.startsWith("houppier") || name.startsWith("racine") || name.startsWith("tronc")){
				removedIndices[j] = i;
				j++;
			} 
...
//		String options = String.format("-t %s -T %s -c %d -x 4 -s 1", 
...
//		Evaluation.evaluateModel(fc, o);
	...
	public String evaluteTestSet() throws Exception{
		...
		sb.append(eval.precision(0) + "\n");
		sb.append(eval.fMeasure(0) + "\n");
//		sb.append(eval.toSummaryString("Test Result", true));
//		sb.append(eval.toClassDetailsString());
//		sb.append(eval.toMatrixString());
//		writeResultToFile(outFile, sb.toString());
		return sb.toString();
	}
